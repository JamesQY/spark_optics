...
(settings: OpticsSettings,
 partitioningSettings: PartitioningSettings = new PartitioningSettings()) extends Serializable {
  ...
  def run(data: RDD[Point]): OpticsModel = {
    ...
    val PointRDD = sc.parallelize(dataPoint, 100).zipWithIndex.partitionBy(new BlockPartitioner(blocks.length))
      .map(x => x._1).cache()
    ...
    val calculateNighborRDD = PointRDD.mapPartitionsWithIndex((partitionIndex, it) => {
      ...
      if (partitionIndex != 0) {
        partitionBeforeBlock = toInstancesBefore(blocks.find(_.blockID == partitionIndex - 1).get.PointArray)
        lnnbefore.setInstances(partitionBeforeBlock)
      }

      val partitionBlock = toInstances(blocks.find(_.blockID == partitionIndex).get.PointArray)
      lnn.setInstances(partitionBlock)

      var partitionAfterBlock: Instances = null
      if (partitionIndex != blocks.length - 1) {
        partitionAfterBlock = toInstancesAfter(blocks.find(_.blockID == partitionIndex + 1).get.PointArray)
    ...
    val TempClusterMap = resignRDD.map(x => (x.ClusterID, 1)).reduceByKey(_ + _).sortBy(_._2, false).collect()
    var i = 0
