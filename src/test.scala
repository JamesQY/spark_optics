...
object test {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("testSpark_Optics").set("spark.akka.frameSize", "2047")
      .set("spark.driver.maxResultSize", "4096m").setMaster("local[*]")
    val sc = new SparkContext(conf)

    val dataSetArray = sc.textFile("./src/result/waveform.csv", 4).map(x => x.split(",", -1).map(_.toDouble)).zipWithIndex()
      .map { x =>
    ...
    val opticsSettings = new OpticsSettings(0.4, 6)
    ...
    //    sc.parallelize(resultString).saveAsTextFile(args(2))
    //    sc.stop()

    val write = new PrintWriter("./src/result/result100SUSY.csv")
    //    write.println("RunningTime:"+opticsModel.runningTime)
    //    write.println("countZeroOne:"+countZeroOne)
    //    write.println("countZeroZero:"+countZeroZero)
    //    write.println("countOneZero:"+countOneZero)
    //    write.println("countOneOne:"+countOneOne)
    //    write.println("persicion:"+persicion)
    write.println(resultString)
    for (elem <- result)
